 ...
 * 
 * It provides: - Validation of course input fields - Adding new course records
 * - Updating existing course records - Fetching course details by ID -
 * Navigation control for course views
 * 
 * Flow: - GET request → loads course data (if id present) and forwards to view
...
public class CourseCtl extends BaseCtl {
	 ...
	 * 
	 * Validation rules: - Name must not be null and must be valid - Duration must
	 * not be null - Description must not be null
	 * 
	...
	protected boolean validate(HttpServletRequest request) {
		log.debug("CourseCtl validate() called");
	...
	protected BaseBean populateBean(HttpServletRequest request) {
		log.debug("CourseCtl populateBean() called");
	 ...
	 * Handles GET request.
	 * 
	 * If id is present, fetches course data and sets it in request scope. Then
	 * forwards to course view.
	 * 
	 * @param request  HttpServletRequest object
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (id > 0) {
			try {
				...
				log.info("Loaded CourseBean for id=" + id);
	 ...
	 * 
	 * Supported operations: - Save → adds new course record - Update → updates
	 * existing course record - Cancel → redirects to course list - Reset → reloads
	 * course form
	 * 
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (OP_SAVE.equalsIgnoreCase(op)) {
			...
			try {
				...
				log.warn("Duplicate course name during update: " + bean.getName());
			} catch (ApplicationException e) {
			...
			log.info("Operation: CANCEL, redirecting to COURSE_LIST_CTL");
			ServletUtility.redirect(ORSView.COURSE_LIST_CTL, request, response);
			return;
		} else if (OP_RESET.equalsIgnoreCase(op)) {
			log.info("Operation: RESET, redirecting to COURSE_CTL");
	...
	protected String getView() {
		log.debug("Returning Course view page");
